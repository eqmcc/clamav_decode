...
int filter_add_static(struct filter *m, const unsigned char *pattern, unsigned long len, const char *name)
{
 //   if(name) if(name[0]=='t') cli_infomsg(NULL,"DEBUG: filter_add_static in as Shift-or FSM\n"); //CHR
    ...
    /*CHR exit condition, one of following two:
        1. find best
        2. scan over whole pattern

        Especially:
        if (best < 100 && j<MAX_CHOICES), not a best choice
        we need best>=100 or j>=MAX_CHOICES
    */
    //CHR max MAX_CHOICES choices
	for(j=0;(best < 100 && j<MAX_CHOICES) || (j < maxlen) ;j++) {
        //CHR looks like num stands for a metric system to evaluate the quality of the qgram
		uint32_t num = MAXSOPATLEN;
		uint8_t k;
		if (j+2 > len)
			break;
        //CHR scan over each MAXSOPATLEN byte length sub pattern
        //CHR there are MAX_CHOICES such sub pattern to scan over
        //CHR each sub pattern starts with one char in first MAX_CHOICES chars in the pattern
        //CHR basicly, the best should got from first (2*MAX_CHOICES-1) chars of the pattern
		for(k=j;k<len-1 && (k-j < MAXSOPATLEN);k++) {
            //CHR k is low 8 bits and k+1 is high 8 bits
            //CHR read in 2 bytes of the pattern as a qgram
         ...
         //    if(name[0]=='t')
           //  cli_infomsg(NULL,"DEBUG: in filter_add_static, read[%c,%c] q=%d\n",pattern[k],pattern[k+1],q);//CHR
            ...
            /*CHR
                ...
                if already have B[q] logged in that pos
                should reduce the probability that this q grama gets picked as best one to add into filter
            CHR*/
			num += filter_isset(m, k-j, q) ? 0 : MAXSOPATLEN - (k-j);
            /*CHR
                if the qgram equals to edge value(0x0000 or 0xffff)
                should signifcantly reduce the probability that this q grama gets picked as best
                increase the num with 10000 or 1000 should do the trick
            CHR*/
			if ((k == j || k == j+1) && (q == 0x0000 || q == 0xffff))
				num += k==j ?  10000 : 1000;/* bad */
           //if(name[0]=='t')cli_infomsg(NULL,"DEBUG: in filter_add_static, num=%d\n",num);//CHR
		}
		/* it is very important to keep the end set small */
        /*CHR
            reduce the probability a little bit(increase by 10) if this qgram already in end set
        CHR*/
		num += 10*(filter_end_isset(m, k-j-1, q) ? 0 : 1);
		/* it is very important to have signatures as long as possible
		 * */
        //CHR the longer the shift offset(which will be selected as a signature in filter) 
        //CHR inside a sub pattern, the greater the probability of being selected
		num += 5*(MAXSOPATLEN - (k-j));
		/* if we are lower length than threshold penalize */
        //CHR shift offset bellow 4 is almost unacceptable in a filter
		if (k-j+1 < 4)
			num += 200;
		/* favour longer patterns */
        //CHR long pattern will be rewarded again
		num -= (2*MAXSOPATLEN - (k + 1+j))*(k-j)/2;

        //CHR mark the best value and position so far
        //CHR smaller the value of num, the better the quality of the signature
		if (num < best) {
			best = num;
			best_pos = j;
		}
 ...
 //   if(name[0]=='t')cli_infomsg(NULL,"DEBUG: in filter_add_static, for %s at m(%p)\n",pattern,m);//CHR
    ...
    //CHR use this as start of pattern
	pattern += best_pos;
	len -= best_pos;
	/* cut length */
	if(len > MAXSOPATLEN) {
