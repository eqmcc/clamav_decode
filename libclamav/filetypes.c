...
cli_file_t cli_filetype2(fmap_t *map, const struct cl_engine *engine)
{
    ...
    cli_infomsg(NULL,"DEBUG: called from cli_filetype2 loc1\n");//CHR
	...
	if(sret >= CL_TYPENO) {
	    ...
	    if(decoded) {
        cli_infomsg(NULL,"DEBUG: called from cli_filetype2 loc2\n");//CHR
		    ...
		    if((encoding = encoding_detect_bom(buff, bread))) {
			    ...
			    if(encoding_normalize_toascii(&in_area, encoding, &out_area) >= 0 && out_area.length > 0) {
				    ...
				    if(out_area.length > 0) {
                        cli_infomsg(NULL,"DEBUG: called from cli_filetype2 loc3\n");//CHR
