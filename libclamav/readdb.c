...
int cli_parse_add(struct cli_matcher *root, const char *virname, const char *hexsig, uint16_t rtype, uint16_t type, const char *offset, uint8_t target, const uint32_t *lsigid, unsigned int options)
{
        ...
        if((wild = strchr(hexsig, '{'))) {
                ...
                //case signature applies BM algo
        } else {
...
static int cli_chkign(const struct cli_matcher *ignored, const char *signame, const char *entry)
{
        ...
        // bm match used in init to identify ignorable ones CHR
...
static int cli_loadcbc(FILE *fs, struct cl_engine *engine, unsigned int *signo, unsigned int options, struct cli_dbio *dbio, const char *dbname)
{
        ...
        cli_infomsg(NULL,"DEBUG: in cli_loadcbc, loading [%s]\n",dbname);//CHR
        ...
        cli_infomsg(NULL,"DEBUG: in cli_loadcbc, loaded [%s] \n",buf);//CHR
        ...
        cli_infomsg(NULL,"DEBUG: in cli_loadcbc, bytecode id=%d and bc->kind=%d for [%s]\n",bc->id,bc->kind,dbname);//CHR
        ...
        if (bc->kind == BC_LOGICAL || bc->lsig) {
                ...
                cli_infomsg(NULL,"Bytecode %s(%u) has logical signature: %s\n", dbname, bc->id, bc->lsig);//CHR
        ...
        sigs++;
        cli_infomsg(NULL,"DEBUG: in cli_loadcbc engine->hook_lsig_ids=%d\n",engine->hook_lsig_ids);//CHR
        if (bc->kind != BC_LOGICAL) {
                ...
                if (bc->kind >= _BC_START_HOOKS && bc->kind < _BC_LAST_HOOK) {
                        ...
                        case BC_STARTUP:
                                cli_infomsg(NULL,"DEBUG: in cli_loadcbc, BC_STARTUP mode\n");
                                for (i=0;i<bcs->count-1;i++)
                                        if (bcs->all_bcs[i].kind == BC_STARTUP) {
                                                struct cli_bc *bc0 = &bcs->all_bcs[i];
                                                cli_infomsg(NULL,"DEBUG: in cli_loadcbc, error with [%d]\n",i);
        ...
        if (signo)
                *signo += sigs;
        cli_infomsg(NULL,"DEBUG: in cli_loadcbc, signo=%d\n",*signo);
        return CL_SUCCESS;
}
...
int cli_load(const char *filename, struct cl_engine *engine, unsigned int *signo, unsigned int options, struct cli_dbio *dbio)
{
        ...
        if(cli_strbcasestr(dbname, ".db")) {
                ...
                //CHR cli_infomsg(NULL,"DEBUG: load hdb: %s\n",dbname);
                                                        ...
                                                        while(!readdir_r(dd, &result.d, &dent) && dent) {
                                                                ...
                                                                while((dent = (struct dirent *) readdir_r(dd, &result.d))) {
                                                                        ...
                                                                        int cl_engine_compile(struct cl_engine *engine)
                                                                        {
                                                                                ...
                                                                                //CHR cli_infomsg(NULL,"DEBUG: BM sigs: %u\n", root->bm_patterns);


                                                                                for(i = 0; i < CLI_MTARGETS; i++) {
                                                                                        if((root = engine->root[i])) {
                                                                                                //CHR cli_infomsg(NULL,"DEBUG: bm_offmode=%d\n", root->bm_offmode);
                                                                                                //CHR cli_infomsg(NULL,"DEBUG: Matcher[%u]: %s: AC sigs: %u (reloff: %u, absoff: %u) BM sigs: %u (reloff: %u, absoff: %u) maxpatlen %u %s\n", i, cli_mtargets[i].name, root->ac_patterns, root->ac_reloff_num, root->ac_absoff_num, root->bm_patterns, root->bm_reloff_num, root->bm_absoff_num, root->maxpatlen, root->ac_only ? "(ac_only mode)" : "");

                                                                                                // CHR prepare for AC algo: build goto/fail tables
                                                                                                if((ret = cli_ac_buildtrie(root)))
                                                                                                        return ret;
                                                                                                cli_dbgmsg("Matcher[%u]: %s: AC sigs: %u (reloff: %u, absoff: %u) BM sigs: %u (reloff: %u, absoff: %u) maxpatlen %u %s\n", i, cli_mtargets[i].name, root->ac_patterns, root->ac_reloff_num, root->ac_absoff_num, root->bm_patterns, root->bm_reloff_num, root->bm_absoff_num, root->maxpatlen, root->ac_only ? "(ac_only mode)" : "");

                                                                                                //CHR
                                                                                                //cli_infomsg(NULL,"Matcher[%u]: %s: AC sigs: %u (reloff: %u, absoff: %u) BM sigs: %u (reloff: %u, absoff: %u) maxpatlen %u %s\n", i, cli_mtargets[i].name, root->ac_patterns, root->ac_reloff_num, root->ac_absoff_num, root->bm_patterns, root->bm_reloff_num, root->bm_absoff_num, root->maxpatlen, root->ac_only ? "(ac_only mode)" : "");
